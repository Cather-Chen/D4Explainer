...
def get_neighbourhood(node_idx, edge_index, features, labels,edge_label_matrix,n_hops):
    ...
    return sub_feat, edge_index_sub, sub_labels,self_label,node_dict, ground_truth, mapping_mask
...
class SynGraphDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        with open(os.path.join(f"./data/{self.name}/raw", f"{self.name}.pkl"), 'rb') as f:
        ...
        for id in range(x.shape[0]):
            sub_feat, edge_index_sub, sub_labels, self_label, node_dict, ground_truth,mapping_mask = get_neighbourhood(id,
                                                                                                          edge_index,
                                                                                                          features=x,
                                                                                                          labels=y,
                                                                                                          edge_label_matrix=edge_label_matrix,
                                                                                                          n_hops=4)
            data = Data(x=sub_feat, edge_index=edge_index_sub, y=sub_labels, self_y=self_label, node_dict=node_dict,
                        ground_truth=ground_truth,mapping = mapping_mask, idx=id)
            print(data)
        ...
        torch.save(self.collate([data_list[i] for i in train_mask]), f"./data/{self.name}/processed/training_sub.pt")
        torch.save(self.collate([data_list[i] for i in val_mask]), f"./data/{self.name}/processed/evaluating_sub.pt")
        torch.save(self.collate([data_list[i] for i in test_mask]), f"./data/{self.name}/processed/testing_sub.pt")
