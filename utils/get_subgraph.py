...
def split_batch(g):
    ...
    num_edges = torch.tensor([e.size(1) for e in edge_indices], dtype=torch.long).to(
        g.x.device
    )
...
def bool_vec(length, r_True, shuffle=True):
    ...
    vec = np.concatenate(
        [np.zeros(n_False, dtype=np.bool), np.ones(n_True, dtype=np.bool)]
    )
    if shuffle:
...
def get_broken_graph(g, broken_ratio, connectivity=True):
    ...
    out_edge_ratio = []
    broken_masks = []
    for edge_index, N, C, E in zip(
        edge_indices, num_nodes.tolist(), cum_nodes.tolist(), num_edges.tolist()
    ):
        if connectivity:
            flag = 0
...
def bid_k_hop_subgraph(
    node_idx, num_hops, edge_index, relabel_nodes=False, num_nodes=None
):
...
def get_syn_ground_truth_graph(g):
    ...
    broken_mask = torch.tensor(
        nodel_label[row] * nodel_label[col] > 0, dtype=torch.bool
    )
...
def get_single_ground_truth_graph(g):
    ...
    for E, C in zip(num_edges.tolist(), cum_edges.tolist()):
        ...
        _, broken_edge_index, _, edge_mask = bid_k_hop_subgraph(
            node_idx, num_hops=5, edge_index=edge_index
        )
