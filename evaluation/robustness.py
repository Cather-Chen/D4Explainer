...
class DiffExplainer(BaseExplainer):
    ...
    def explain_graph(self, model, graph, adj_b, x_b, node_flag_b, sigma_list, args):
        ...
        # _, _, _, noisy_adj_b, _ = gen_list_of_data_single(x_b, adj_b, node_flag_b, [mod_ratio], args)
        #
        # mask = generate_mask(node_flag_b).to(self.device)
        # score = self.model(A=noisy_adj_b, node_features=x_b, mask=mask, noiselevel=mod_ratio).to(self.device)
...
for args.dataset in ["NCI1"]:
    ...
    for sigma in range(args.start, 11):
        sigma /= 100
        acc_logger = []
        orig_modif_r_arr = []
        noisy_modif_r_arr = []
        ...
        for graph in tqdm(iter(test_loader), total=len(test_loader)):
            ...
            if args.explainer == "DiffExplainer":
                ...
                # top_k_orig_exp_edges = graph.edge_index[:, np.argsort(-orig_subgraph_imp)[:args.k]]
                ...
                # noisy_exp_subgraph = Explainer.pack_explanatory_subgraph(args.mod_ratio, graph=noisy_graph, imp=noisy_edge_imp, if_cf=True)
                # noisy_exp_pred = get_graph_pred(noisy_exp_subgraph)

            noisy_graph_same_class.append(1 if orig_pred == noisy_pred else 0)
            # if orig_pred == noisy_pred:
            #     acc_logger.append(1 if noisy_exp_pred != orig_pred else 0)
