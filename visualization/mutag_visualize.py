...
def visualize(
    ...
    if dataset == "ba3":
        ...
        ################################
        diff_adj = to_dense_adj(
            to_undirected(edge_index_diff), max_num_nodes=graph.x.size(0)
        )
        ori_adj = to_dense_adj(
            to_undirected(graph.edge_index), max_num_nodes=graph.x.size(0)
        )
        add_adj = diff_adj - ori_adj
        ...
        plt.title(
            f"label: {label.item()}; mr(cf)={vis_ratio}; pred(cf):{pred_cf.item()}; mr(diff)={modification_ratio}; pred(diff)={pred_diff.item()}",
            fontsize=10,
        )
        if save:
            plt.savefig(folder / Path(f"{graph.name[0]}.pdf"), dpi=500)
