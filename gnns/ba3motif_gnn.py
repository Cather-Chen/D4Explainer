...
def parse_args():
    ...
    parser.add_argument(
        "--data_path",
        nargs="?",
        default=osp.join(osp.dirname(__file__), "..", "data", "BA3"),
...
class BA3MotifNet_attr(torch.nn.Module):
    def __init__(self, num_unit):
        super().__init__()
...
if __name__ == "__main__":
    ...
    model = (
        BA3MotifNet_attr(args.num_unit).to(device)
        if args.with_attr
        else BA3MotifNet(args.num_unit).to(device)
    )
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        _, train_acc = Gtest(
            train_loader, model, device=device, criterion=CrossEntropyLoss()
        )

        val_error, val_acc = Gtest(
            val_loader, model, device=device, criterion=CrossEntropyLoss()
        )
        test_error, test_acc = Gtest(
            test_loader, model, device=device, criterion=CrossEntropyLoss()
        )
