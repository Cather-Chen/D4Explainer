...
def parse_args():
    ...
    parser.add_argument('--data_name', nargs='?', default="Cornell",
    ...
    parser.add_argument('--num_unit', type=int, default=6,
...
class EGNN(nn.Module):
    ...
    def get_pred_explain(self, x, edge_index, edge_mask, mapping=None):
        ...
        for i in range(self.num_layers):
            ...
            residual_weight = self.c_min - self.beta
            x = self.layers_GCN[i](x, edge_index, original_x,
                                   beta=self.beta, residual_weight=residual_weight, edge_weight=edge_mask)
            x = self.layers_activation[i](x)
        x = F.dropout(x, p=self.output_dropout, training=self.training)
        node_repr = self.output_layer(x)
        node_prob = F.softmax(node_repr,dim=-1)
...
class EGNN_attr(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr=None):
        ...
        for i in range(self.num_layers):
            ...
            x = self.layers_GCN[i](x, edge_index, original_x, edge_weight=edge_weight,
                                   beta=self.beta, residual_weight=residual_weight)
            # x = self.layers_bn[i](x)
            x = self.layers_activation[i](x)
...
if __name__ == '__main__':
    ...
    if args.with_attr:
        model = EGNN_attr(n_input, hidden_channels=args.hidden, num_classes=n_labels, num_layers=args.num_unit).to(device)
    else:
