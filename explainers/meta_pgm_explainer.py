...
class MetaPGMExplainer:
    ...
    def batch_perturb_features_on_node(self, num_samples, index_to_perturb,
        ...
        if self.task == "nc":
            soft_pred, _ = self.model.get_node_pred_subgraph(x=self.graph.x, edge_index=self.graph.edge_index, mapping=self.graph.mapping)
        else:
        ...
        pred_label = self.graph.y.detach().cpu().numpy() if self.task == "gc" else self.graph.self_y.detach().cpu().numpy()
        ...
        for iteration in range(num_samples):
            ...
            tmp_g.x = X_perturb
            if self.task == "nc":
                soft_pred_perturb, _ = self.model.get_node_pred_subgraph(x=tmp_g.x, edge_index=tmp_g.edge_index,
                                                                 mapping=tmp_g.mapping)
            else:
                soft_pred_perturb, _ = self.model.get_pred(x=tmp_g.x, edge_index=tmp_g.edge_index,
                                                   batch=tmp_g.batch)  #[bsz, num_class]
