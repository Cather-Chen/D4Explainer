...
class Explainer(object):
    ...
    def evaluate_acc(self, top_ratio_list, graph=None, imp=None, if_cf=False):
        ...
        fidelity = np.array([[]])
        if self.task == "nc":
            output_prob, _ = self.model.get_node_pred_subgraph(x=graph.x, edge_index=graph.edge_index,
                                                             mapping=graph.mapping)
        else:
            output_prob, _ = self.model.get_pred(x=graph.x, edge_index=graph.edge_index,
                                               batch=graph.batch)
        ...
        for idx, top_ratio in enumerate(top_ratio_list):
            ...
            if self.task == "nc":
                ...
                soft_pred, _ = self.model.get_pred(x=exp_subgraph.x, edge_index=exp_subgraph.edge_index, batch=exp_subgraph.batch)
