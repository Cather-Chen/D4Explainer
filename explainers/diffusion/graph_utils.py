...
def graph2tensor(graph, device):
    ...
    adj = to_dense_adj(
        edge_index, batch=graph.batch
    )  # [bsz, max_num_node, max_num_node]
...
def tensor2graph(graph_batch, score, mask_adj, threshold=0.5):
    ...
    node_sizes = (
        degree(graph_batch.batch, dtype=torch.long).detach().cpu().numpy()
    )  # list of node numbers
    sum_list = torch.tensor([node_sizes[:i].sum() for i in range(bsz)]).to(
        score_tensor.device
    )
...
def gen_full(batch, mask):
    ...
    bsz = mask.size(0)
    node_sizes = (
        degree(batch, dtype=torch.long).detach().cpu().numpy()
    )  # list of node numbers
