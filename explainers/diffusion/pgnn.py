...
def zero_diag(matrix: torch.Tensor):
    matrix = matrix * (
        1 - torch.eye(matrix.size(-2), matrix.size(-1), device=matrix.device)
    )
    return matrix
...
def masked_instance_norm2D(x: torch.Tensor, mask: torch.Tensor, eps: float = 1e-5):
    ...
    var = (
        torch.sum(var_term, dim=[1, 2]) / (torch.sum(mask, dim=[1, 2])) + 1e-5
    )  # (N,C)
    # var = var#.detach()
...
class PowerfulLayer(nn.Module):
    ...
    def __init__(
        self, in_feat: int, out_feat: int, num_layers: int, spectral_norm=(lambda x: x)
    ):
        super().__init__()
...
class Powerful(nn.Module):
    def __init__(
        ...
        if self.cat_output:
            if self.project_first:
                self.layer_cat_lin = nn.Sequential(
                    spectral_norm(
                        nn.Linear(self.hidden * (self.num_layers + 1), self.hidden)
                    )
                )
            else:
                self.layer_cat_lin = nn.Sequential(
                    spectral_norm(
                        nn.Linear(
                            self.hidden * self.num_layers + self.input_features,
                            self.hidden,
                        )
                    )
                )
        ...
        self.final_lin = nn.Sequential(
            spectral_norm(nn.Linear(self.hidden, self.output_features))
        )
        ...
        if self.node_out:
            ...
            self.final_lin_node = nn.Sequential(
                spectral_norm(nn.Linear(self.hidden, self.node_output_features))
            )

        self.test_lin = nn.Sequential(
            spectral_norm(
                nn.Linear(self.input_features, self.output_features, bias=False)
            )
        )
    ...
    def forward_cat(self, A, node_features, mask, noiselevel):
        ...
        #     """
        node_feature1 = node_features.unsqueeze(1).repeat(
            1, node_features.size(1), 1, 1
        )
        node_feature2 = node_features.unsqueeze(2).repeat(
            1, 1, node_features.size(1), 1
        )
        ...
        u = torch.cat([u, node_feature1, node_feature2, noise_level_matrix], dim=-1).to(
            self.device
        )  # [bsz, N, N, 2C+2]
