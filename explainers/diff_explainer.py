...
def loss_cf_exp(
    gnn_model, graph_batch, score, y_pred, y_exp, full_edge, mask, ds, task="nc"
):
    ...
    neg_prop = (output_repr_cont.unsqueeze(1).expand(bsz, n, n) + inf_diag).logsumexp(
        -1
    ) - output_repr_cont.logsumexp(-1).unsqueeze(1).repeat(1, n)
...
class DiffExplainer(Explainer):
    ...
    def train(self, args, model, gnn_model, train_dataset, test_dataset):
        ...
        for epoch in range(args.epoch):
            ...
            for i, graph in enumerate(train_loader):
                ...
                train_node_flag_b = (
                    train_adj_b.sum(-1).gt(1e-5).to(dtype=torch.float32)
                )  # [bsz, N]
                ...
                optimizer.zero_grad()
                train_noise_adj_b_chunked = train_noise_adj_b.chunk(
                    len(sigma_list), dim=0
                )
                train_x_b_chunked = train_x_b.chunk(len(sigma_list), dim=0)
                train_node_flag_sigma = train_node_flag_sigma.chunk(
                    len(sigma_list), dim=0
                )
                score = []
                masks = []
                ...
                y_pred, y_exp = gnn_pred(
                    graph, graph_batch_sub, gnn_model, ds=args.dataset, task=args.task
                )
                full_edge_index = gen_full(graph.batch, mask)
                score_b = (
                    torch.cat(score, dim=0).squeeze(-1).to(args.device)
                )  # [len(sigma_list)*bsz, N, N]
                masktens = torch.cat(masks, dim=0).to(
                    args.device
                )  # [len(sigma_list)*bsz, N, N]
            ...
            if (epoch + 1) % args.verbose == 0:
                ...
                test_loader = DataLoader(
                    dataset=test_dataset, batch_size=args.test_batchsize, shuffle=False
                )
                model.eval()
                for graph in test_loader:
                    ...
                    test_node_flag_b = (
                        test_adj_b.sum(-1).gt(1e-5).to(dtype=torch.float32)
                    )
                    sigma_list = (
                        list(
                            np.random.uniform(
                    ...
                    with torch.no_grad():
                        test_noise_adj_b_chunked = test_noise_adj_b.chunk(
                            len(sigma_list), dim=0
                        )
                        test_x_b_chunked = test_x_b.chunk(len(sigma_list), dim=0)
                        test_node_flag_sigma = test_node_flag_sigma.chunk(
                            len(sigma_list), dim=0
                        )
                        score = []
                        masks = []
                ...
                if mean_test_sparsity < best_sparsity:
                    ...
                    model_save(
                        args, model, mean_train_loss, best_sparsity, mean_test_acc
                    )
    ...
    def explain_evaluation(self, args, graph):
        ...
        model.load_state_dict(
            torch.load(os.path.join(exp_dir, "best_model.pth"))["model"]
        )
        model.eval()
        graph.to(args.device)
        ...
        full_edge_index = gen_full(graph.batch, mask)
        modif_r = sparsity(score, test_adj_b, mask)
        score_tensor = torch.stack(score, dim=0).squeeze(
            -1
        )  # len_sigma_list, bsz, N, N]
        score_tensor = torch.mean(score_tensor, dim=0).view(-1, 1)  # [bsz*N*N,1]
        mask_bool = mask.bool().view(-1, 1)
        edge_mask_full = score_tensor[mask_bool]
        if args.task == "nc":
            output_prob_cont, output_repr_cont = self.model.get_pred_explain(
                x=graph.x,
    ...
    def model_level_explain(self, args, random_adj, node_feature, sigma):
        ...
        model.eval()
        score = model(
            A=random_adj, node_features=node_feature, mask=mask, noiselevel=sigma
        ).to(
            args.device
        )  # [1, N, N, 1]
