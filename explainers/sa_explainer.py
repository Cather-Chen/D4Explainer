...
class SAExplainer(Explainer):
    ...
    def explain_graph(self, graph, model=None, draw_graph=0, vis_ratio=0.2):
        """
        Explain a graph using SAExplainer
        :param graph: graph to be explained
        :param model: model to be used for explanation
        :param draw_graph: whether to draw the graph
        :param vis_ratio: ratio of edges to be visualized
        ...
        if self.task == "nc":
            soft_pred, _ = model.get_node_pred_subgraph(
                x=tmp_graph.x, edge_index=tmp_graph.edge_index, edge_attr=tmp_graph.edge_attr, mapping=tmp_graph.mapping
            )
        else:
            soft_pred, _ = model.get_pred(
                x=tmp_graph.x, edge_index=tmp_graph.edge_index, edge_attr=tmp_graph.edge_attr, batch=tmp_graph.batch
            )
