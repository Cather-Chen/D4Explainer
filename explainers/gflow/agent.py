...
class GraphAgent(nn.Module):
    ...
    def __init__(self, n_conv, n_hidden, n_out_stem, n_out_graph, n_input):
        ...
        # self.conv = nn.ModuleList([gnn.GCNConv(nn.Sequential(nn.Linear(n_hidden, 2*n_hidden),
        #                                        nn.Linear(2*n_hidden, n_hidden))) for _ in range(n_conv)])


        # self.block2emb = nn.Sequential(nn.Linear(n_hidden, n_hidden),
        #                                nn.ReLU(),
        #                                nn.Linear(n_hidden, n_hidden)
        #                                )
        # self.gru = nn.GRU(n_hidden, n_hidden)
        # self.stem2pred = nn.Sequential(nn.Linear(n_hidden * 2, n_hidden),
        #                                nn.ReLU(),
        #                                nn.Linear(n_hidden, n_hidden),
        #                                nn.ReLU(),
        #                                nn.Linear(n_hidden, n_out_stem)
        #                                )
        # self.global2pred = nn.Sequential(nn.Linear(n_hidden, n_hidden),
        #                                  nn.ReLU(),
        #                                  nn.Linear(n_hidden, n_out_graph)
        #                                  )
    ...
    def forward(self, graph, state, full_graph, gnn_model, actions=None):
        ...
        # block_linear, bond_linear, full_bond_linear = self.linear_layers
        #
        # out = block_linear(graph.x)
        # edge_attr = bond_linear(graph.edge_attr)  #(n,n_hidden)
        ...
        # Index of the origin block of each stem in the batch (each stem is a pair 
        # [block idx, stem atom type], we need to adjust for the batch packing)
        node_reps = gnn_model.get_node_reps(graph).detach() # (mbsize *n_node, n_hidden)
    ...
    def foward_multisteps(self, graph, gnn_model, remove_ratio=0.5):
        ...
        n_edge_remove = graph.num_edges - 1 # int(remove_ratio * graph.num_edges)
