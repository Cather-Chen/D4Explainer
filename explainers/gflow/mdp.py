...
from utils.tensor import bool2str
...
class GraphMDP(gym.Env):
    ...
    def __init__(self, graph, target_model, device,
        ...
        self.cf_flag = False if target_y is None else True
        if target_y is None:
            self.target_pred = F.softmax(target_model(graph)).detach()
        else:
            self.target_pred = torch.zeros_like(target_model(graph))
            self.target_pred[target_y] = 1.
    ...
    def _get_reward(self, state, pre_reward=0, mode='mutual_info'):
        ...
        ##########################Note Below###########
        if self.cf_flag:
            # for counterfactual explanations, we use the logits at the target class as reward
            reward = torch.sum(self.target_y * torch.log(pred + 1), dim=1)
        else:
