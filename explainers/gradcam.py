...
class GradCam(Explainer):
    ...
    def __init__(self, device, gnn_model_path, task,ds):
        super(GradCam, self).__init__(device, gnn_model_path, task)
        self.ds = ds
    ...
    def explain_graph(self, graph,
        ...
        if self.task == "nc":
            soft_pred, _ = model.get_node_pred_subgraph(x=tmp_graph.x, edge_index=tmp_graph.edge_index, edge_attr =tmp_graph.edge_attr, mapping=tmp_graph.mapping)
        else:
            soft_pred, _ = model.get_pred(x=tmp_graph.x, edge_index=tmp_graph.edge_index, edge_attr=tmp_graph.edge_attr, batch=tmp_graph.batch)
        ...
        if draw_graph:
            self.visualize(tmp_graph, edge_score, self.name, vis_ratio=vis_ratio)
        self.last_result = (tmp_graph, edge_score)
